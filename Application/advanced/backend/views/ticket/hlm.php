...
<?php  
 ...
 $query = "Select req_name, count(ticket.request_req_id) as count from request left join ticket on(request.req_id=ticket.request_req_id) where ticket.ticket_type_id IN('3', '4') AND tick_startDate BETWEEN DATE_ADD(NOW(), INTERVAL -1 MONTH) AND NOW() group by request.req_id ORDER BY COUNT DESC";   
 $result = mysqli_query($connect, $query); 

 $query2 = "Select req_name, count(ticket.request_req_id) as count from request left join ticket on(request.req_id=ticket.request_req_id) where ticket.ticket_type_id IN('3', '4')  AND tick_startDate BETWEEN DATE_ADD(NOW(), INTERVAL -1 MONTH) AND NOW() group by request.req_id ORDER BY COUNT DESC limit 3";  
 $result2 = mysqli_query($connect, $query2);

 $query6 = "Select room_location , count(room.room_no) as count from room left join ticket on (room.room_no = ticket.room_room_no) where ticket_type_id in ('3','4') AND tick_startDate BETWEEN DATE_ADD(NOW(), INTERVAL -1 MONTH) AND NOW() group by room.room_location order by count desc";  
...
$query3 = "Select name, count(user.id) as count from user left join ticket on (user.id = ticket.assigned_to) where ticket_type_id in ('3', '4') AND tick_startDate BETWEEN DATE_ADD(NOW(), INTERVAL -1 MONTH) AND NOW() group by user.id order by count desc limit 5";
...
$query4 = "Select name, count(user.id) as count from user left join ticket on (user.id = ticket.assigned_to) where ticket_type_id in ('3', '4') AND tick_startDate BETWEEN DATE_ADD(NOW(), INTERVAL -1 MONTH) AND NOW() group by user.id order by count desc limit 5"; 
...
$query5 = "select room_room_no, count(*) as count from ticket where ticket_type_id in ('3', '4') AND tick_startDate BETWEEN DATE_ADD(NOW(), INTERVAL -1 MONTH) AND NOW() group by room_room_no order by count desc limit 5";
